...
// 8. 'ErrorContainer'という名前のインターフェースを定義します。このインターフェースは、文字列型のプロパティ名をキーとし、文字列を値とするオブジェクトを表します。

// 9. 'ErrorContainer'型の新しいオブジェクト'errorBag'を作成します。このオブジェクトは、'email'プロパティに'正しいメールアドレスではありません'、'username'プロパティに'ユーザー名に記号を含めることはあできません'を持つように初期化します。
